...
def get_prediction(img: np.ndarray) -> dict[str, int | float]:
    ...
    if res.status_code != 200:
        raise ValueError(f"Backend {PREDICTION_ENDPOINT} returned error {res.status_code}: {res.text}")
    return res.json()
...
def put_submission(
    ...
    if res.status_code != 200:
        raise ValueError(f"Backend {SUBMISSION_ENDPOINT} returned error {res.status_code}: {res.text}")
    return res.json()
